import React, { useEffect, useState } from 'react';
import "./Nav.css";
import { Link } from 'react-router-dom';


    function Nav() {
        const [show, handleShow] = useState(false);
      
        useEffect(() => {
          const handleScroll = () => {
            if (window.scrollY > 100) {
              handleShow(true);
            } else {
              handleShow(false);
            }
          };
      
          window.addEventListener("scroll", handleScroll);
      
          return () => {
            window.removeEventListener("scroll", handleScroll); // Pass the same event listener function to removeEventListener
          };
        }, []);
      
        // Rest of your component code
  return (
    <div className={`nav ${show && "nav_black"}`}>
      
       <img className="nav_logo" src="data:image/png;base64," alt="Netflix Logo"/>
       
       <Link to ="/">
       <button className="logout-button">Logout</button>
       </Link>
    </div>
  );
}

export default Nav;